...
export class ExtractionDataComponent implements OnInit {
  ...
  tables: any[] = ['Communes', 'Comptes Utilisateur', 'Favoris', 'Fils de conversation', "Notifications", "Rubriques"];
  ...
  schemaMessage = ["id", "texte", "heure", "lu"];
  ...
  factory() {
    ...
    //if (this.tableSelected == "Messages") this.messageService.get().subscribe(res => this.download(res, this.schemaFavori));
    if (this.tableSelected == "Notifications") this.notifService.get().subscribe(res => this.download(res, this.schemaMessage));
